...
A contract which serves data to client contract(s). Suppose a client contract want to live score. It will query that data from Oracle contract by sending the name of asset need to get and Oracle contract will revert the data to client's entrypoint with requested data. 
